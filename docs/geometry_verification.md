...
A cylindrical face does not prove that it is a hole, boss, groove, pattern
member, or a particular FeaturePlan child. Therefore holes, bosses, grooves,
patterns, fillets, chamfers, ribs, sketches, and reference relationships have
registered but `UNSUPPORTED` verification until a later phase supplies reliable
geometry and correspondence evidence. When an isolated base has follow-up
features, its direct base check is also `UNKNOWN` rather than guessing how the
final BRep relates to the original primitive.
