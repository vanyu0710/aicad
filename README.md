...
## 使用流程（v0.10 对话式 harness）

1. **打开界面**，`新建项目`。
2. 在右侧 **AI 助手** 聊天框直接下指令（如"120×120×12 法兰，中心 Ø30 通孔，6 个 Ø8 螺栓孔在 Ø90 圆上"），
   或在左侧表单写描述后点 **Agent 建模** / `Ctrl+G`——所有入口都汇聚为**同一条会话消息**。
   首条消息可自动携带左侧上传的**草图图片**（vision 进 agent 上下文）。
3. Agent 在**会话流**里工作：模型文字**逐字流式**显示，每个工具调用是一张内嵌卡片（op / 参数 / 结果）；
   3D 视口随几何变化刷新。
4. **运行中也能插话**：聊天框始终可用，消息排队后在当前步骤结束、下一轮模型决策前生效。
5. 遇到需要你决定的事，会话流顶部弹出**审批卡**：
...
8. **会话持久化**：每个项目一条 agent 会话（`work/agent_sessions/{id}.json`），重开项目可回看历史；
